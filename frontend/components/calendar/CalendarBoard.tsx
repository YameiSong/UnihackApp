"use client";
import React from "react";
import Calendar from 'react-calendar';
import 'react-calendar/dist/Calendar.css';
import { useRef, useState } from 'react';
import { useGlobalContext } from "../providers/GlobalStateProvider";
import Tag from "../tag/Tag";

import {
  Dialog,
  DialogContent,
  DialogDescription,
  DialogHeader,
  DialogTitle,
  DialogTrigger,
} from "@/components/ui/dialog"
import {
  Card,
  CardContent,
  CardDescription,
  CardHeader,
  CardTitle,
} from "@/components/ui/card";
import {
  Command,
  CommandEmpty,
  CommandGroup,
  CommandInput,
  CommandItem,
} from "@/components/ui/command"
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
} from "@/components/ui/popover"
import { Button } from "../ui/button";



const CalendarBoard = () => {
  const { tags, tagTimes, setTagTimes, tagColours } = useGlobalContext();
  const [openTagChoices, setOpenTagChoices] = useState(false);
  const [currentTagChoice, setCurrentTagChoice] = useState();
  const dialogTriggerRef = useRef(null);
  const addTagTimeTimeRef = useRef(null);
  let selectedDate = new Date();

  const addTagTime = () => {
    dialogTriggerRef.current.click()
    const [hoursString, minutesString] = addTagTimeTimeRef.current.value.split(':');
    selectedDate.setHours(parseInt(hoursString));
    selectedDate.setMinutes(parseInt(minutesString));
    const tagTime = {
      tag: currentTagChoice,
      time: selectedDate
    }
    console.log(tagTime)
  }

  const handleClickDay = (value, e) => {
    dialogTriggerRef.current.click()
    selectedDate = value;
  }
  return (
    <>
      <Dialog>
        <DialogTrigger className='hidden' ref={dialogTriggerRef}>Close</DialogTrigger>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className='text-center'>Select the tag and time</DialogTitle>
            <DialogDescription>
              <div className="flex flex-col gap-y-4">
              <div className="justify-around flex">
                <Popover open={openTagChoices} onOpenChange={setOpenTagChoices}>
                  <PopoverTrigger asChild>
                    <Button
                      variant="outline"
                      role="combobox"
                      aria-expanded={openTagChoices}
                      className="w-[200px] justify-between drop-shadow-lg p-2 flex items-center"
                    >
                      {currentTagChoice ?
                        <Tag value={currentTagChoice.value} backgroundColour={tagColours[tags.indexOf(currentTagChoice) % tagColours.length]} canDelete={false}>

                        </Tag> : "Select tag"}
                      {!openTagChoices ? <svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M4.18179 6.18181C4.35753 6.00608 4.64245 6.00608 4.81819 6.18181L7.49999 8.86362L10.1818 6.18181C10.3575 6.00608 10.6424 6.00608 10.8182 6.18181C10.9939 6.35755 10.9939 6.64247 10.8182 6.81821L7.81819 9.81821C7.73379 9.9026 7.61934 9.95001 7.49999 9.95001C7.38064 9.95001 7.26618 9.9026 7.18179 9.81821L4.18179 6.81821C4.00605 6.64247 4.00605 6.35755 4.18179 6.18181Z" fill="currentColor" fill-rule="evenodd" clip-rule="evenodd"></path></svg>
                        : <svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3.13523 8.84197C3.3241 9.04343 3.64052 9.05363 3.84197 8.86477L7.5 5.43536L11.158 8.86477C11.3595 9.05363 11.6759 9.04343 11.8648 8.84197C12.0536 8.64051 12.0434 8.32409 11.842 8.13523L7.84197 4.38523C7.64964 4.20492 7.35036 4.20492 7.15803 4.38523L3.15803 8.13523C2.95657 8.32409 2.94637 8.64051 3.13523 8.84197Z" fill="currentColor" fill-rule="evenodd" clip-rule="evenodd"></path></svg>
                      }

                    </Button>
                  </PopoverTrigger>
                  <PopoverContent>
                    <Command>
                      <CommandInput placeholder="Search Tag..." />
                      <CommandEmpty>No tags created.</CommandEmpty>
                      <CommandGroup>
                        {tags.map((tag, index) => (
                          <CommandItem key={tag.value} value={tag} onSelect={() => {
                            setCurrentTagChoice(tag);
                            setOpenTagChoices(false);
                          }}>
                            <Tag value={tag.value} key={index} backgroundColour={tagColours[index % tagColours.length]} canDelete={false}></Tag>
                          </CommandItem>
                        ))}
                      </CommandGroup>
                    </Command>

                  </PopoverContent>
                </Popover>
                <input ref={addTagTimeTimeRef} type="time" className=" drop-shadow-lg p-2"></input>
              </div>
              <div className="flex justify-center">
                <button className="ring ring-2 rounded-lg p-2 ring-gray-400" onClick={addTagTime}>Add tag and time</button>
              </div>
              
              </div>
            </DialogDescription>
          </DialogHeader>
        </DialogContent>
      </Dialog>
      <Card className="min-h-[180px] shadow-md">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-lg font-bold">Calendar</CardTitle>
        </CardHeader>
        <CardContent className="flex justify-center">
          <Calendar onClickDay={handleClickDay} />
        </CardContent>
      </Card>
    </>

  );
};

export default CalendarBoard;
